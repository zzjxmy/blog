...
class Worker
{
    ...
    public static function runAllWorkers()
    {
        ...
        {
            throw new Exception("@@@no worker inited@@@\r\n");
        }
    }
    ...
    public static function openProcess($start_file)
    {
	    ...
	    // 将stdout(标准输出)  stderr(标准错误) 重定向到文件 所有的读写从文件中
        $descriptorspec = array(
                0 => array('pipe', 'a'), // stdin 标准输入
                1 => array('file', $std_file, 'w'), // stdout 标准输出
                2 => array('file', $std_file, 'w') // stderr 标准错误
        );
        ...
        // 打开stdout(标准输出) stderr(标准错误) 文件句柄(资源文件)
        $std_handler = fopen($std_file, 'a+');
        //  为资源流设置阻塞或者阻塞模式
        stream_set_blocking($std_handler, 0);
        // 定时读取子进程的stdout(标准输出) stderr(标准错误)
	    //每0.1秒去读取进程的标准输入数据
    ...
    {
        // 保存worker实例 spl_object_hash 获取对象的hash值
